...
{	
	...
	unsigned char *data = stbi_load("Texturas/texturaMadera.png", &width, &height, &nrChannels, 0);
	if (data)
	{
		// IMPORTANT note
...
void display(void)
{
	...
	glBindTexture(GL_TEXTURE_2D, texture2);
	projectionShader.setInt("texture2", 0);
	model = glm::translate(model, glm::vec3(0.0f, -0.3999f, 0.0f));
